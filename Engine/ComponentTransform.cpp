...
void ComponentTransform::AddTransform(const math::float4x4& transform)
{
	...
	transform.Decompose(scaling, aiRotation, scaling);
...
void ComponentTransform::DrawProperties()
{
	...
	{
		if (ImGui::DragFloat3("Position", (float*)&position, 0.1f, -1000.0f))
		{
			edited = true;
		}
		...
		iF8ImGui::DragFloat3("scale", (float*)&scale, 0.1f, 0.01f, 100.0f))
		{
		edited = true;
		}
